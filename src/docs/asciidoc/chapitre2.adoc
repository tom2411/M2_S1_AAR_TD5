...
public class TodosServiceImpl {
    ...
    * ATTENTION : pour un Prototype cette méthode ne sera pas appelée («fire and forget») 
...
[source,java]
ApplicationContext ctx = new FileSystemXmlApplicationContext("/home/application/config.xml");

    * Pour lire plusieurs fichiers (varargs) : 
[source,java]
ApplicationContext ctx = new FileSystemXmlApplicationContext("conf-1.xml" ,"conf-2.xml");
xml
    * Pour lire tous les fichiers d’un répertoire (pattern Ant) : 
[source,java]
ApplicationContext ctx = new FileSystemXmlApplicationContext("/home/application/*.xml");
...
----
    * Note : cette configuration lance uniquement Spring IoC, sans Spring MVC 
